...
def step(hparams, agent, state, env, worker_id, recurrent_state=None):
  """ run envrionment for one step and return the output """
  if hparams.render:
    env.render()

  if type(env).__name__ == 'NavRLEnv':
    action, recurrent_state = agent.act(state, worker_id,
  ...
  return action, reward, done, state, recurrent_state
  ...
  for i in range(hparams.test_episodes):
    ...
    while not done:
      if hparams.render:
        env.render()
      last_state = state
      action, reward, done, state = step(hparams,
                                         agent,
                                         last_state,
                                         env,
                                         worker_id=0)
        ...
        if hparams.num_workers == 1:
          ...
          workers = [
              threading.Thread(target=train,
                               args=(worker_id, agent, hparams, checkpoint))
